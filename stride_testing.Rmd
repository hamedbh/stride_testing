...
cv_list <- map(seq_len(nrow(xgb_grid)), function(i) {
    a <- xgb_grid[i, ][["eta"]]
    b <- xgb_grid[i, ][["max_depth"]]
    
    xgb.cv(params = list(eta = a, 
                         max_depth = b, 
...
```{r}
errors <- map_dbl(seq_along(cv_list), function(i) {
    min(cv_list[[i]][["evaluation_log"]][["test_error_mean"]])
})
best_cv <- cv_list[[which.min(errors)]]
(best_params <- xgb_grid[which.min(errors), ])
```

```{r}
best_cv[["evaluation_log"]] %>% 
...
my_xgb <- xgb.train(params = list(eta = best_params[["eta"]], 
